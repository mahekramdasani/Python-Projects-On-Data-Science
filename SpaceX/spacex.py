...
def update_scatter_chart(selected_site, payload_range):
    if selected_site == 'ALL':
        ...
        title = f'Scatter Plot for All Sites (Payload Range: {payload_range[0]} - {payload_range[1]} kg)'
    else:
        # Filter data based on payload range for the selected site
        filtered_df = spacex_df[(spacex_df['Launch Site'] == selected_site) &
                                (spacex_df['Payload Mass (kg)'].between(payload_range[0], payload_range[1]))]
        title = f'Scatter Plot for {selected_site} (Payload Range: {payload_range[0]} - {payload_range[1]} kg)'
